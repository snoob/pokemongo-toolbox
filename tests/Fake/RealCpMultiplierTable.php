<?php

declare(strict_types=1);

namespace App\Tests\Fake;

use App\Domain\Pvp\Model\PokemonLevel;
use App\Domain\Pvp\Port\CpMultiplierTable;
use App\Infrastructure\Json\JsonValue;

/**
 * The real CP multipliers, inlined so unit tests stay free of disk and network.
 * Regenerate with "bin/console pogo:data:build" if the game ever changes them.
 */
final class RealCpMultiplierTable implements CpMultiplierTable
{
    private const array MULTIPLIERS = [
        '1' => 0.094,
        '1.5' => 0.13513743215803847,
        '2' => 0.16639787,
        '2.5' => 0.19265091454861796,
        '3' => 0.21573247,
        '3.5' => 0.23657265541932715,
        '4' => 0.25572005,
        '4.5' => 0.27353037931097973,
        '5' => 0.29024988,
        '5.5' => 0.30605738000722543,
        '6' => 0.3210876,
        '6.5' => 0.3354450348019347,
        '7' => 0.34921268,
        '7.5' => 0.36245775711118555,
        '8' => 0.3752356,
        '8.5' => 0.3875924191428145,
        '9' => 0.39956728,
        '9.5' => 0.4111935439951595,
        '10' => 0.4225,
        '10.5' => 0.4329264087965774,
        '11' => 0.44310755,
        '11.5' => 0.4530599628689135,
        '12' => 0.4627984,
        '12.5' => 0.4723360827308573,
        '13' => 0.48168495,
        '13.5' => 0.49085580932476297,
        '14' => 0.49985844,
        '14.5' => 0.5087017591555174,
        '15' => 0.51739395,
        '15.5' => 0.5259424956328841,
        '16' => 0.5343543,
        '16.5' => 0.5426357508963908,
        '17' => 0.5507927,
        '17.5' => 0.5588305922386229,
        '18' => 0.5667545,
        '18.5' => 0.574569134506658,
        '19' => 0.5822789,
        '19.5' => 0.5898879034974399,
        '20' => 0.5974,
        '20.5' => 0.6048236602280411,
        '21' => 0.6121573,
        '21.5' => 0.6194041050661919,
        '22' => 0.6265671,
        '22.5' => 0.6336491667895227,
        '23' => 0.64065295,
        '23.5' => 0.6475809587060136,
        '24' => 0.65443563,
        '24.5' => 0.6612192609753201,
        '25' => 0.667934,
        '25.5' => 0.6745818887829742,
        '26' => 0.6811649,
        '26.5' => 0.6876848943474521,
        '27' => 0.69414365,
        '27.5' => 0.7005428891384746,
        '28' => 0.7068842,
        '28.5' => 0.713169102419072,
        '29' => 0.7193991,
        '29.5' => 0.7255756180718899,
        '30' => 0.7317,
        '30.5' => 0.7347410173422504,
        '31' => 0.7377695,
        '31.5' => 0.7407855800803546,
        '32' => 0.74378943,
        '32.5' => 0.7467812039953893,
        '33' => 0.74976104,
        '33.5' => 0.7527290986842915,
        '34' => 0.7556855,
        '34.5' => 0.7586303636507689,
        '35' => 0.76156384,
        '35.5' => 0.7644860688461087,
        '36' => 0.76739717,
        '36.5' => 0.7702972738840048,
        '37' => 0.7731865,
        '37.5' => 0.7760649434180147,
        '38' => 0.77893275,
        '38.5' => 0.7817900775756758,
        '39' => 0.784637,
        '39.5' => 0.7874735905949481,
        '40' => 0.7903,
        '40.5' => 0.7928039417157309,
        '41' => 0.7953,
        '41.5' => 0.7978039170121942,
        '42' => 0.8003,
        '42.5' => 0.8028038926163724,
        '43' => 0.8053,
        '43.5' => 0.8078038685225517,
        '44' => 0.8103,
        '44.5' => 0.8128038447251588,
        '45' => 0.8153,
        '45.5' => 0.8178038212187566,
        '46' => 0.8203,
        '46.5' => 0.8228037979980404,
        '47' => 0.8253,
        '47.5' => 0.8278037750578334,
        '48' => 0.8303,
        '48.5' => 0.8328037523930834,
        '49' => 0.8353,
        '49.5' => 0.8378037299988584,
        '50' => 0.8403,
        '50.5' => 0.842803707870344,
        '51' => 0.8453,
    ];

    #[\Override]
    public function multiplierFor(PokemonLevel $level): float
    {
        $multiplier = JsonValue::asFloatOrNull(self::MULTIPLIERS[(string) $level] ?? null);

        if (null === $multiplier) {
            throw new \RuntimeException(\sprintf('No multiplier for level %s.', $level));
        }

        return $multiplier;
    }
}
